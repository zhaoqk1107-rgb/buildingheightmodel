...
class FeatureAggregator(nn.Module):
    def __init__(self, hidden_dim=516, out_dim=256, nq=200):
        super().__init__()
        self.aggregator = SelfAttentionAggregator(hidden_dim, out_dim, nq=nq) # 2) Intrainstance Attention:
        self.spatial_rel_attn = GCN(nfeat=out_dim, nhid=out_dim * 2) # 3) Interinstance Attention Graph:
    ...
    def forward(self, pred_height, pred_logits, pred_masks, mask_embed):
        """
        Args:
            pred_height: [B, N, 256] （隐藏层特征）
            pred_logits: [B, N, 2]
            pred_masks: [B, N, H, W]
            mask_embed: [B, N, 256]
        """
        mask_centers = calculate_instance_centers(pred_masks)
        multi_feats = torch.concat([pred_height, pred_logits, mask_centers, mask_embed], dim=-1) # (B, 200(nq), 516)
        height_feat_intra = self.aggregator(multi_feats).squeeze(0)  # [n, nq, 256]
        height_feat_gcn = self.spatial_rel_attn(height_feat_intra) # [n, nq, 256] 根据论文
        # === 修改：添加残差连接 ===
        # 在早期训练阶段，如果GCN因为Mask不准而产生噪声，残差连接允许原始特征直接流过
        # 相当于 height_feat = original + gcn(original)
        height_feat = self.norm_res(height_feat_intra + height_feat_gcn)
        height_feat_res = self.ffn(height_feat) # [n, nq, 256]
...
if __name__ == "__main__":
    ...
    y = model(pred_logits, pred_masks)
    print(y.shape)
    print("Done!")
